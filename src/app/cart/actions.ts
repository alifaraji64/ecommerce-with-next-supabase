...
export async function createInvoice(price_amount: string, products: string, quantities: string) {
    const res = await fetch("https://payid19.com/api/v1/create_invoice", {
        ...
        body: JSON.stringify({
            ...
            test: 1,
            price_amount,
            success_url: `${process.env.NEXT_PUBLIC_BASE_URL}/add-order?price=${price_amount}&products=${products}&quantities=${quantities}`,
        }),
    });
