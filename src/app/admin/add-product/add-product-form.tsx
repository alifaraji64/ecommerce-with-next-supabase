        ...
        <form action={formAction} className='max-w-lg mx-auto mt-8 grid gap-28'>
            <div className="grid gap-12">
                <div className="grid gap-2">
                    ...
                    <input defaultValue={state.values.name} type="text" name="name" id="name" className='w-full border border-gray-300 rounded-md p-2' />
                </div>
                <div className="grid gap-2">
                    <label htmlFor="description">Product Description</label>
                    {state.errors.description && <p className='text-red-500'>{state.errors.description}</p>}
                    <textarea defaultValue={state.values.description} name="description" id="description" className='w-full border border-gray-300 rounded-md p-2' />
                </div>
                <div className="grid gap-2">
                    <label htmlFor="price">Product Price</label>
                    {state.errors.price && <p className='text-red-500'>{state.errors.price}</p>}
                    <input defaultValue={state.values.price} type="number" name="price" id="price" className='w-full border border-gray-300 rounded-md p-2' />
                </div>
                <div className="grid gap-2">
                    <label htmlFor="quantity">Product Quantity</label>
                    {state.errors.quantity && <p className='text-red-500'>{state.errors.quantity}</p>}
                    <input defaultValue={state.values.quantity} type="number" name="quantity" id="quantity" className='w-full border border-gray-300 rounded-md p-2' />
                </div>
                <div className="grid gap-2">
                    <label htmlFor="category">Category</label>
                    {state.errors.category && <p className='text-red-500'>{state.errors.category}</p>}
                    <select defaultValue={state.values.category} defaultChecked={false} name="category" id="category" className='w-full border border-gray-300 rounded-md p-2'>
